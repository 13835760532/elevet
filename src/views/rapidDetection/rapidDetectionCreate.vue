...
/**
 * 生成检测记录接口入参。
 * 深拷贝用于隔离格式转换对响应式表单的影响，后续接口重试不会改变用户正在编辑的内容。
 * 样品来源在页面中是多选数组，后端存储为逗号文本；总体结论则统一映射为
 * `0=阴性/合格`、`1=阳性/不合格`、`2=结果异常`。路由 id 在任务新建场景表示 taskId，
 * 在 detect/recheck 场景表示既有检测记录，具体读取分支由 route.query.action 决定。
 */
const getSubmitData = () => {
...
/**
 * 推进四步检测流程。
 * 第一步校验并预存样品记录，先获得记录主键，避免试纸识别和报告生成成为无主数据；
 * 第二步必须有完整的 AI 识别 JSON 才允许写入结果；第三步只在备注变化时更新备注，再生成报告；
 * 第四步为只读预览。复检场景在结果保存阶段改用复检接口且不会重复创建产品档案，
 * 任一步接口失败都停留在当前步骤，保留用户已填写的表单内容以便重试。
 */
 ...
 * 页面展示的简要结论仅用于交互反馈，不能替代原始 JSON；报告页需要原始 results、时间戳和
 * 试纸图片等字段还原检测现场，因此识别成功后同时保存机器可读和人可读两种数据。
 */
...
/**
 * 完成检测记录存档。
 * 第一步已经创建过主记录时只更新，避免重复生成样品编号；复检操作使用专用接口，避免覆盖原检测链路。
 * 成功后返回自主检测列表，列表会按接口最终状态显示公开性、复检标记和检测结论。
 */
 ...
 * 新建场景从第一步开始，继续检测和复检直接进入试纸识别步骤；taskId 新建场景则读取任务默认的
 * 区划、主体、品种、标准和方法，但不创建检测记录，仍由第一步预存操作统一生成主键。
 * 这样可避免“任务下发信息”与“具体样品检测结果”被混为同一条记录。
 */
onMounted(async () => {
    // 预加载
